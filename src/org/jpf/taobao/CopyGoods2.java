...
import com.asiainfo.utils.AiZipUtil;
...
public class CopyGoods2 extends Thread {
    ...
    int ShopCount = 1;
    //最大标题数量
    private final int MAX_TITLE_COUNT=100; 
    /**
    ...
    public void writelog() {
        ...
        TaoBaoUtils.appendCsv(strLogFileName, AiDateTimeUtil.getCurrDate() + "\t" + strCvsFilePath + "\t"
        ...
        if (file.isDirectory()) {
            ...
            for (File fileSec1 : files1) {
                ...
                for (File fileSec2 : files2) {
                    ...
                    AiZipUtil.zipMultiFile(fileSec2.getAbsolutePath(), strZipFileName, true);
                }
            }
        }
        /*
     ...
     */
    private void distribution(StringBuilder sbCvsFileTitle, Vector<String> vTitles, String strKey, String strCvsName)
            throws Exception {
    ...
    private void writeToCsv(String strCvsName, StringBuilder sb, int iShopNumber) throws Exception {
        String strNewCvsName = MakeOutPutDir(strCvsName, iShopNumber) + AiFileUtil.getFileName(strCvsName);
        //logger.info(strNewCvsName);
        TaoBaoUtils.writeToCsv(strNewCvsName, sb);
     ...
     */
    /*
    private String showPicPath(final String strPicInfo,String strCvsName) {
        String strKey="file:///";
        String strTmpPicInfo = strPicInfo;
        int i = strTmpPicInfo.indexOf(strKey);
        if (i > 0) {
            strTmpPicInfo = strTmpPicInfo.substring(i+strKey.length(), strPicInfo.length());
            i = strTmpPicInfo.indexOf(">");
            strTmpPicInfo = strTmpPicInfo.substring(0, i-1);
            i = strTmpPicInfo.lastIndexOf("/");
            //图片中位置
            String strOldPicPath = strTmpPicInfo.substring(0, i);
            int j = strTmpPicInfo.indexOf("\"");
            String strPicName = strTmpPicInfo.substring(i+1, j);
            
            String strExistPath=AiFileUtil.FileExist(getCurrentGoodPath(strCvsName), strPicName);
            if (strExistPath!=null) 
            {
                String strNewPicPath=strConstPicPath+strExistPath.substring(strCvsFilePath.length(),strExistPath.length());
                strNewPicPath=AiFileUtil.getFilePath(strNewPicPath);
                logger.debug("strNewPicPath:" + strNewPicPath);
                logger.debug("strOldPicPath:" + strOldPicPath);
                strNewPicPath=strNewPicPath.replaceAll("\\\\", "/");
                logger.info("strNewPicPath:" + strNewPicPath);
                logger.info("strOldPicPath:" + strOldPicPath);
                //TextAreaLogAppender.log("strConstPicPath:" + strConstPicPath);
                //TextAreaLogAppender.log("strTmpPicInfo:" + strTmpPicInfo);
                //logger.info(strPicInfo.indexOf(strPicPath));
                strTmpPicInfo =AiStringUtil.ReplaceAll (strPicInfo,strOldPicPath, strNewPicPath);
                
                logger.info(strTmpPicInfo);
    ...
    */
    ...
    public String MakeOutPutDir(String strCvsFileName, int iShopNumber) throws Exception {
        try {
            ...
            // 复制图片目录
            String strShopDir = OUTPUT_PATH + "\\" + iShopNumber + "\\" + sourceDir + "\\" + sourceFileName;
